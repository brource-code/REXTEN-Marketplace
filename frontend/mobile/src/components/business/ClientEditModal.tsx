...
import { updateClient, BusinessClient, CreateClientPayload } from '../../api/business';
...
export function ClientEditModal({ visible, onClose, client, onSuccess }: Props) {
  ...
  useEffect(() => {
    if (client) {
      ...
      setAddress('');
      const normalizedStatus = (client.status || 'regular').replace('_client', '') as ClientStatus;
      setStatus(['regular', 'permanent', 'vip'].includes(normalizedStatus) ? normalizedStatus : 'regular');
    }
  }, [client]);
  ...
  const mutation = useMutation({
    mutationFn: (data: Partial<CreateClientPayload>) => {
  ...
  const handleSubmit = useCallback(() => {
    ...
    const isPlaceholderEmail =
      email && (email.includes('@local.local') || !email.includes('@') || email.trim() === '');

    mutation.mutate({
      name: name.trim(),
      email: email.trim() && !isPlaceholderEmail ? email.trim() : undefined,
        ...
        <View style={[styles.footer, { borderTopColor: colors.border }]}>
          ...
          >
            {mutation.isPending ? (
              ...
              <Text style={[styles.submitBtnText, { color: colors.buttonText }]}>{T.save}</Text>
            )}
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>

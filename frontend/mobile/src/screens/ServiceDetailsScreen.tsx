...
export const ServiceDetailsScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated || !isClient || !profile?.service?.id) {
      setFavoriteServices([]);
    ...
    loadFavs();
  }, [isAuthenticated, isClient, profile?.service?.id]);
  ...
  }, [favoriteServices, favoriteAdvertisements, profile?.service?.id, isAuthenticated, isClient]);
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.imageContainer, { backgroundColor: colors.backgroundSecondary }]}>
            {normalizeImageUrl(business.imageUrl) ? (
              <Image
                source={{ uri: normalizeImageUrl(business.imageUrl)! }}
                style={styles.mainImage}
                resizeMode="cover"
              />
            ) : (
              <View style={styles.noImageContainer}>
                <Text style={[styles.noImageText, { color: colors.textMuted }]}>Нет изображения</Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.contentContainer}>
            ...
            {/* Избранное — только для роли CLIENT (маршруты /client/*) */}
            ...
            <TouchableOpacity
              ...
              style={{
                ...
                paddingVertical: 8,
                borderRadius: 8,
                backgroundColor: isFavorite ? colors.errorLight : colors.backgroundTertiary,
                marginTop: 12,
                opacity: isFavoriteLoading ? 0.6 : 1,
              }}
            >
              {isFavoriteLoading ? (
                <ActivityIndicator size="small" color={isFavorite ? colors.error : colors.textSecondary} style={{ marginRight: 6 }} />
              ) : (
                <Ionicons
              ...
              <View style={styles.priceContainer}>
                ...
                {services[0].duration && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                    ...
                    <Text style={[styles.priceLabelText, { color: colors.textSecondary }]}>
          ...
          <ScrollView
            ...
            style={[styles.tabsContainer, { borderBottomColor: colors.border, backgroundColor: colors.background }]}
            ...
            {tabs.map((tab) => (
              ...
              >
                <View 
                  style={[
                    styles.tabButton,
                    activeTab === tab.id ? { borderBottomColor: colors.primary } : styles.tabButtonInactive
                  ]}
                >
                  <Text
                    style={[
          ...
          <View style={styles.tabContent}>
              ...
              <View>
                ...
                {services.length > 0 && (
                  <View style={{ marginTop: 24 }}>
                    <Text style={[styles.sectionHeader, { color: colors.text }]}>Доступные услуги</Text>
                    {services.map((service) => (
                      <View key={service.id} style={[styles.serviceItemContainer, { borderBottomColor: colors.border }]}>
                        <View style={styles.serviceItemHeader}>
                          <Text style={[styles.serviceItemName, { color: colors.text }]}>{service.name}</Text>
                          {service.price && (
                            <Text style={[styles.serviceItemPrice, { color: colors.text }]}>
                              {service.price}
                            </Text>
                          )}
                        </View>
                        {service.description && (
                          <Text style={[styles.serviceItemDescription, { color: colors.textSecondary }]}>
                            {service.description}
                          </Text>
                        )}
                        {service.duration && (
                          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            ...
                            <Text style={[styles.serviceItemDuration, { color: colors.textSecondary }]}>
            ...
            {activeTab === 'reviews' && (
              <View>
                <Text style={[styles.sectionHeader, { color: colors.text }]}>Отзывы</Text>
                {reviews.length === 0 ? (
            ...
            {activeTab === 'team' && (
              <View>
                <Text style={[styles.sectionHeader, { color: colors.text }]}>Наша команда</Text>
                {team.length === 0 ? (
                    ...
                    <View key={member.id} style={[styles.teamMemberContainer, { borderBottomColor: colors.border }]}>
                      <View style={[styles.teamMemberAvatar, { backgroundColor: colors.textMuted }]}>
                        {normalizeImageUrl(member.avatarUrl) ? (
                          ...
                          <Text style={[styles.teamMemberAvatarText, { color: colors.buttonText }]}>
            ...
            {activeTab === 'portfolio' && (
              <View>
                <Text style={[styles.sectionHeader, { color: colors.text }]}>Портфолио</Text>
                {portfolio.length === 0 ? (
                    ...
                    {portfolio.map((item, idx) => (
                      ...
                      >
                        <Image
                          ...
                          style={[styles.portfolioImage, { backgroundColor: colors.backgroundSecondary }]}
                          resizeMode="cover"
                        />
                        {item.title && (
                          <Text style={[styles.portfolioTitle, { color: colors.text }]} numberOfLines={1}>
                            {item.title}
                          </Text>
                        )}
                      </TouchableOpacity>
        ...
        >
          <View style={[styles.modalContainer, { backgroundColor: colors.background }]}>
            {selectedImageIndex !== null && portfolio[selectedImageIndex] && (
              <>
                <Image
                  source={{ uri: normalizeImageUrl(portfolio[selectedImageIndex].imageUrl)! }}
                ...
                {portfolio[selectedImageIndex].title && (
                  <View style={styles.modalImageTitle}>
                    <Text style={[styles.modalImageTitleText, { color: colors.buttonText }]}>
                      {portfolio[selectedImageIndex].title}
                    </Text>
                  </View>
                )}
        ...
        <View style={[styles.bookButtonContainer, { borderTopColor: colors.border, backgroundColor: colors.background }]}>
          <TouchableOpacity
            onPress={() => {

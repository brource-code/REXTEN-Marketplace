...
// Для бизнеса: элементы из папки "Бизнес" без папки (плоский список)
// Для суперадмина: папка "Бизнес" остается как есть
...
const businessNavigationFlatConfig = (businessNavigationConfig[0]?.subMenu || []).map(item => ({
    ...item,
    authority: [BUSINESS_OWNER], // Только для бизнеса
}))

// Бизнес - для суперадмина (с папкой)
// Видно только для SUPERADMIN
const businessNavigationForSuperadminConfig = businessNavigationConfig.map(item => ({
    ...item,
    authority: [SUPERADMIN], // Только для суперадмина
}))
...
const navigationConfig = [
    ...
    // Для суперадмина: папка "Бизнес" - видно только SUPERADMIN
    ...businessNavigationForSuperadminConfig,
    // clientNavigationConfig убран - клиенты не должны быть в админке
    // Для суперадмина: папка "Суперадмин"

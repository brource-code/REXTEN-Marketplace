...
const ToastWrapper = (props) => {
    ...
    const messageElements = messages.map((item) => {
        // Если item.node - это объект (не React элемент), преобразуем его в Notification компонент
        let node = item.node
        if (!isValidElement(node) && typeof node === 'object' && node !== null) {
            node = createElement(Notification, {
                title: node.title,
        ...
        return (
            ...
            >
                {isValidElement(node) ? cloneElement(node, {
                    ...toastProps,
                    ref,
                    onClose: chainedFunction(node?.props?.onClose, () =>
                        remove(item.key),
                    ),

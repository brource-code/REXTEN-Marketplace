    ...
    public function getServiceProfile($slug)
    {
        ...
        if ($advertisement) {
            ...
            $adReviews = Review::where('advertisement_id', $advertisement->id)
                ->where('is_visible', true)
                ->with('user.profile', 'service', 'booking.specialist.profile', 'order.booking.specialist.profile', 'booking')
                ->get();
            ...
            $reviews = $adReviews->map(function ($review) use ($advertisement) {
                ...
                if ($review->user_id) {
                    ...
                    if ($review->booking && $review->booking->client_name) {
                        $userName = $review->booking->client_name;
                    } elseif ($review->order && $review->order->booking && $review->order->booking->client_name) {
                        $userName = $review->order->booking->client_name;
                    }
                }
                ...
                // Если есть специалист и объявление, ищем его в команде объявления
                if ($bookingSpecialistId && $advertisement && $advertisement->team) {
                ...
                // Fallback: если не нашли в команде, но есть специалист, используем его имя из профиля
                if (!$specialistName && $bookingSpecialistId) {
                    ...
                    if ($specialist) {
                        ...
                        if ($specialistProfile && $specialistProfile->first_name && $specialistProfile->last_name) {
                            $specialistName = trim($specialistProfile->first_name . ' ' . $specialistProfile->last_name);
                        } elseif ($specialist->name && $specialist->name !== 'Администратор Системы') {
                            // Используем имя пользователя только если это не дефолтное имя
                ...
                return [
                    'id' => (string) $review->id,
                    'userName' => $userName,
                    'userAvatar' => $review->user_id ? ($review->user->profile->avatar ?? null) : null,
        ...
        $serviceReviews = $service->reviews()->where('is_visible', true)->with('user.profile', 'service', 'booking.specialist.profile', 'order.booking.specialist.profile', 'booking')->get();

...
class BookingsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Booking::where('user_id', $user->id)
            ->with(['service', 'company', 'specialist', 'additionalServices']);
        ...
        $data = $bookings->map(function ($booking) {
            ...
            if ($booking->specialist_id) {
                // Сначала пробуем найти мастера в команде объявления
                ...
                // Если не нашли в команде объявления, используем пользователя из таблицы users
                if (!$specialistName && $booking->specialist) {
                    $specialistName = $booking->specialist->profile->full_name ?? $booking->specialist->name ?? null;
                }
            }
            ...
            $basePrice = (float) ($booking->price ?? $booking->total_price ?? 0);

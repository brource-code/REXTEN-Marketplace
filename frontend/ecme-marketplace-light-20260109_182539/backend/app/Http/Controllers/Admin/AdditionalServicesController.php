...
class AdditionalServicesController extends Controller
{
    ...
    {
        $serviceId = $request->get('service_id'); // ID услуги из БД (может быть число) или из JSON (может быть строка)
        
        // Если передан service_id, ищем дополнительные услуги по service_id
        // (так как дополнительные услуги привязаны к услуге, а не напрямую к объявлению)
        if ($serviceId) {
            ...
            if (is_numeric($serviceId) && (int)$serviceId > 0) {
                ...
                $query->where(function($q) use ($serviceId) {
                    $q->where('service_id', (int)$serviceId)
                      ->orWhere(function($q2) {
                          // Общие услуги для всех (без привязки к конкретной услуге)
                          $q2->whereNull('service_id')->whereNull('service_json_id');
                      });
                });
            } else {
                // Виртуальная услуга из JSON - фильтруем по service_json_id
                $query->where(function($q) use ($serviceId) {
            ...
            // Если service_id не передан, ищем по advertisement_id (старая логика для обратной совместимости)

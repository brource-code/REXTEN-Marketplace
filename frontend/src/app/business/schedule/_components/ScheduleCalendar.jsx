...
const ScheduleCalendar = ({ initialSlots = [] }) => {
    ...
    const updateSlotMutation = useMutation({
            ...
            if (isCompleted && hasReviewToken && selectedSlot) {
                ...
                if (updatedSlot) {
                    ...
                    setSelectedSlot(prev => ({
                        ...
                        has_client_account: responseData.has_client_account, // Добавляем флаг наличия клиентского аккаунта
        ...
        onError: (error) => {
            console.error('[updateSlotMutation] onError:', {
    ...
    const handleEventClick = (arg) => {
        ...
        if (slot) {
            console.log('[ScheduleCalendar] handleEventClick - slot data:', {
                id: slot.id,
                status: slot.status,
                user_id: slot.user_id,
                has_client_account: slot.has_client_account,
    ...
    const handleSubmit = (data, type) => {
        if (type === 'NEW') {
            ...
            createSlotMutation.mutate({
                ...
                title: data.title || null, // Название для произвольных событий
                booking_date: data.booking_date,
                booking_time: data.booking_time,
                duration_minutes: data.duration_minutes,
                status: data.status,
                ...
                price: data.title ? (Math.round(Number(data.price ?? 0) * 100) / 100) : servicePrice, // Для произвольных событий — введённая цена, округлённая до 2 знаков
                additional_services: data.additional_services || [],
                // Передаем адрес клиента для offsite бронирований

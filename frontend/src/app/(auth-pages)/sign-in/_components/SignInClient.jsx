...
const SignInClient = () => {
    ...
    useEffect(() => {
        ...
        if (searchParams.get('reason') === 'registration_closed') {
            registrationNoticeShown.current = true
            toast.push(
                <Notification title={tSignIn('title')} type="warning" width={350}>
                    {tSignIn('registrationClosedNotice')}
                </Notification>,
                    ...
                    if (!error?.response && error?.request) {
                        ...
                        if (responseData.code === 'email_not_verified') {
                            ...
                            const is403Status =
                                ...
                                status === 403 ||
                                statusString === '403' ||
                                String(status).trim() === '403'
                            const hasBlockedText =
                                responseMessageLower.includes('заблокирован') ||

...
const VerticalMenuContent = (props) => {
    ...
    useEffect(() => {
        if (activedRoute?.parentKeys && activedRoute.parentKeys.length > 0) {
            setDefaulExpandKey(activedRoute.parentKeys)
        } else if (activedRoute?.parentKey) {
            setDefaulExpandKey([activedRoute.parentKey])
        }
    }, [activedRoute?.parentKey, activedRoute?.parentKeys])

...
export function normalizeImageUrl(imageUrl) {
    ...
    if (imageUrl.startsWith('http://') || imageUrl.startsWith('https://')) {
        try {
            ...
            // Если URL содержит IP адрес или localhost, заменяем его на правильный домен
            // Это решает проблему с недоступными IP адресами
            if (hostname.match(/^\d+\.\d+\.\d+\.\d+$/) || hostname === 'localhost' || hostname === '127.0.0.1') {
                // Если путь начинается с /storage/, используем Next.js API route как прокси
                if (path.startsWith('/storage/')) {
                    const normalized = `/api${path}`
                    if (process.env.NODE_ENV === 'development') {
                        console.log('[normalizeImageUrl] Replacing IP/localhost with Next.js proxy:', {
                            original: imageUrl,
                            normalized,
                            path,
                            hostname
...
export function denormalizeImageUrl(imageUrl) {
    ...
    // Если URL начинается с /api/storage/, заменяем на полный URL Laravel API
    if (imageUrl.startsWith('/api/storage/')) {
        const path = imageUrl.replace('/api/storage', '/storage')
        const apiUrl = process.env.NEXT_PUBLIC_LARAVEL_API_URL || process.env.NEXT_PUBLIC_API_URL || 'https://api.rexten.live'
        const baseUrl = apiUrl.replace(/\/api\/?$/, '')
        return `${baseUrl}${path}`
    }

    // Если URL уже полный или относительный от Laravel, возвращаем как есть
    return imageUrl
}

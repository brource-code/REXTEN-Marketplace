# Инструкция по установке Ecme Marketplace на сервер
...
sudo git clone https://github.com/your-username/ecme-marketplace.git
cd ecme-marketplace/backend

# Или загрузите архив и распакуйте
# sudo unzip ecme-marketplace.zip -d /var/www
```

### 2. Установка зависимостей

```bash
cd /var/www/ecme-marketplace/backend
...
sudo chown -R www-data:www-data /var/www/ecme-marketplace/backend

# Устанавливаем права
sudo chmod -R 755 /var/www/ecme-marketplace/backend
sudo chmod -R 775 /var/www/ecme-marketplace/backend/storage
sudo chmod -R 775 /var/www/ecme-marketplace/backend/bootstrap/cache
...
```bash
sudo chown -R www-data:www-data /var/www/ecme-marketplace
sudo chmod -R 755 /var/www/ecme-marketplace
```
...
server {
    ...
    root /var/www/ecme-marketplace/backend/public;
    ...
    location /storage {
        alias /var/www/ecme-marketplace/backend/storage/app/public;
...
server {
    ...
    root /var/www/ecme-marketplace/.next;
    ...
    location /_next/static {
        alias /var/www/ecme-marketplace/.next/static;
        expires 365d;
...
command=php /var/www/ecme-marketplace/backend/artisan queue:work --sleep=3 --tries=3 --max-time=3600
autostart=true
autorestart=true
stopasgroup=true
killasgroup=true
...
stdout_logfile=/var/www/ecme-marketplace/backend/storage/logs/queue.log
stopwaitsecs=3600
```
...
```ini
[Unit]
Description=Ecme Marketplace Backend
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/var/www/ecme-marketplace/backend
...
```bash
# Laravel логи
tail -f /var/www/ecme-marketplace/backend/storage/logs/laravel.log
...
0 * * * * cd /var/www/ecme-marketplace/backend && php artisan cache:clear

# Оптимизация Laravel каждый день в 3:00
0 3 * * * cd /var/www/ecme-marketplace/backend && php artisan optimize:clear && php artisan optimize
...
```bash
nano /var/www/ecme-marketplace/backend/config/cors.php
```
...
# Бэкап файлов
tar -czf "$BACKUP_DIR/files_$DATE.tar.gz" /var/www/ecme-marketplace/backend/storage
...
nano /var/www/ecme-marketplace/backend/.env
...
sudo chown -R www-data:www-data /var/www/ecme-marketplace/.next
sudo chmod -R 755 /var/www/ecme-marketplace/.next
...
Добавьте в `/var/www/ecme-marketplace/.env.local`:
...
Добавьте в `/var/www/ecme-marketplace/backend/.env`:
...
1. Проверьте логи: `/var/www/ecme-marketplace/backend/storage/logs/laravel.log`

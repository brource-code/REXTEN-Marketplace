    ...
    const handleToggleFavorite = async (e) => {
        ...
        try {
                    ...
                    if (addError?.response?.status === 401) {
                        toast.push(
                            <Notification
                                ...
                                type="info"
                            >
                                Пожалуйста, войдите, чтобы добавлять в избранное
                            </Notification>
                        )
                        setIsFavoriteLoading(false)
                        return
            ...
            if (error?.response?.status === 401) {
                ...
                toast.push({
                    title: 'Ошибка',
                    message: 'Элемент не найден. Возможно, он был удален.',
                    type: 'danger',
                })
            } else {
                toast.push({
                    title: 'Ошибка',
                    message: error.message || 'Не удалось обновить избранное. Попробуйте позже.',
                    type: 'danger',
                })
            }
        } finally {
            ...
            >
                ...
                <div className="relative w-28 h-24 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800">
                    {service?.imageUrl ? (
                        <img
                            ...
                            onError={(e) => {
                                // Если изображение не загрузилось, пробуем нормализовать еще раз
                                // Это может помочь, если URL был неправильно нормализован
                                const originalUrl = service.imageUrl
                                if (originalUrl && !originalUrl.startsWith('http')) {
                                    const normalized = normalizeImageUrl(originalUrl)
                                    if (normalized && normalized !== e.target.src) {
                                        e.target.src = normalized
                                        e.target.onerror = null
                                        return
                                    }
                                }
                                // Если и нормализованное не загрузилось, показываем placeholder
                ...
                <div className="flex-1 flex flex-col gap-1.5 min-w-0">
                    ...
                    <p className="text-[11px] text-gray-500 dark:text-gray-400">
                        {service.category || service.groupLabel || 'Услуга'}
                    </p>
